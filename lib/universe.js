import { celestialBodyCompositions } from './planet-composition.js?v=1';
import { getFormulaColor } from './formula-registry.js?v=1';
import { formatFormulaWithSubscripts } from './formula-utils.js?v=1';
import { planetsData, sunData } from './planets-data.js?v=1';
...
export function initUniverse() {
	...
	// Configure Paper.js to not handle touch events itself
	// This allows our custom touch handlers to work
	...
	function updateBodyStates() {
		...
		if (simulationState.centerBodyTarget) {
			...
			if (simulationState.centerBodyTarget.body) {
				...
				centerBodyType = celestialBodyStates.get(centerBodyName)?.type || 'planet';
			} else {
				centerBodyName = 'Sun';
				centerBodyType = 'sun';
			}
		...
		celestialBodyStates.forEach((state) => {
			...
			if (state.opacity < state.targetOpacity) {
				state.opacity = Math.min(state.targetOpacity, state.opacity + fadeSpeed);
				anyChanging = true;
			} else if (state.opacity > state.targetOpacity) {
				state.opacity = Math.max(state.targetOpacity, state.opacity - fadeSpeed);
				anyChanging = true;
			}
	...
	function setCenterBody(body, parentBody = null) {
		...
		if (simulationState.centerBody && simulationState.centerBody.body === body) {
			return;
		}
	...
	function drawLabel(config) {
		...
		if (currentLayer && ['planet', 'moon'].includes(bodyType)) {
			...
			if (hasCompositionData) {
				...
				const layerOffset = new paper.Point(10, simulationState.labelFontSize + 5);
	...
	function updatePlanets() {
		...
		planetsData.forEach((planet, planetIndex) => {
			...
			const scaledSize = Math.max(effectiveSize * simulationState.scale, minSize);
	...
	function animate() {
		...
		const needsRedraw = isHovering || bodyHoverStates.size > 0 || statesChanging;
	...
	function drawCompositionDiagram(clickCircle, bodyName) {
		...
		const planetData = celestialBodyCompositions.find((p) => p.name === bodyName);
		if (!planetData) {
		...
		const currentDrawAngle = startAngle + (endAngle - startAngle) * drawProgress;
			...
			segments.forEach((seg) => {
				...
				if (currentDrawAngle > seg.startAngle) {
					...
					if (compound && (compound.formula || compound.compound)) {
						const formulaLabel = new paper.PointText(new paper.Point(labelX, labelY));
	...
	function handleMove(event) {
			...
			if (hitResult.item.data?.isClickArea) {
				...
				clickAreaGroup.children.forEach((child) => {
					if (child.data?.isClickArea && child.data?.type === bodyType) {
						...
						const distance = child.position.getDistance(hitResult.item.position);
						if (distance < 5) {
		...
		if (isPinching && event.touches && event.touches.length === 2) {
			...
			// Calculate zoom factor (adjust sensitivity here)
			const zoomSensitivity = 0.005;
			const zoomDelta = deltaDistance * zoomSensitivity;

			// Update scale with constraints (min 0.1, max 10)
			let newScale = simulationState.scale + zoomDelta;
			newScale = Math.max(0.1, Math.min(10, newScale));

			if (simulationState.scale !== newScale) {
				simulationState.scale = newScale;
